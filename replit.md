...
- **Platform-wide Text Overflow Prevention** (2025-11-24): Global CSS rules ensure all text elements (p, span, div, a, button, label, input, textarea) use word-wrap and overflow-wrap to prevent text from overflowing containers. Long email addresses, URLs, and text content automatically wrap to fit within their boundaries.
- **Equal Height Service Cards** (2025-11-24): All service cards maintain consistent heights regardless of title length or pricing type through min-height constraints on title sections (min-h-[3.5rem] for full cards, min-h-[2.5rem] for compact) and pricing sections (min-h-[4.5rem]). This ensures clean grid alignment.
- **Hashtag System** (2025-11-24): Services support both tags and hashtags fields. Hashtags are clickable links on service detail pages that navigate to hashtag results pages showing all services with that hashtag. The hashtag search uses SQL array matching (`= ANY()`) on the hashtags field. Services must have hashtags populated in the database for them to appear in search results.
